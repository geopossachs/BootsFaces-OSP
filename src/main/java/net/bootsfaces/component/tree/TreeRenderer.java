...
public class TreeRenderer extends CoreRenderer {
	...
	public void encodeEnd(FacesContext context, UIComponent component) throws IOException {
		...
		// per default ajax behavior is disabled
		boolean activateAjaxBehavior = false;

		//FIXME - can we remove this next line of code?
		// String updateItems = BsfUtils.GetOrDefault("'" + tree.getUpdate() + "'", "null");
		String updateItems = tree.getUpdate();
		if (updateItems != null) {
			activateAjaxBehavior = true;
			updateItems = ExpressionResolver.getComponentIDs(context, component, updateItems);
		}

		// make the secound check for ajax behavior
		if (tree.getNodeSelectionListener() != null && !activateAjaxBehavior) {
			activateAjaxBehavior = true;
		}

		final UIForm form = BsfUtils.getClosestForm(tree);
		if (activateAjaxBehavior && form == null) {
			// we need a <form> for ajax call's
			throw new FacesException("The tree component must be inside a form", null);
		}
		...
		rw.writeText("function getTreeData() { "
			...
			+ "   data: getTreeData(),   "
			+ (activateAjaxBehavior ? ( // enable nodeSelected event callback
